...
Fuzzy_C_Means_S1 *Fuzzy_C_Means_S1::instance = 0;

Fuzzy_C_Means_S1 *Fuzzy_C_Means_S1::getInstance() {
    if (!instance)
        instance = new Fuzzy_C_Means_S1();

    return instance;
}

Fuzzy_C_Means_S1::Fuzzy_C_Means_S1() {
}

Fuzzy_C_Means_S1::~Fuzzy_C_Means_S1() {
}

void Fuzzy_C_Means_S1::padMatrix(Mat &m, int n, Mat &out) {
...
void Fuzzy_C_Means_S1::printFloatMatrix(cv::Mat imgMat) {
...
void Fuzzy_C_Means_S1::padMatrix_1(Mat &m, Mat &outModified) {
...
void Fuzzy_C_Means_S1::udDist_FCM_S1(Mat &center, Mat &data, Mat &dm1, float a, Mat &out) {
...
/*        for (int jj = 0; jj < data.rows; jj++) {
            out.at<uchar>(ii, jj) = pow((abs(center.at<uchar>(ii, jj) - data.at<uchar>(0, jj))), 2) +
                                    (a * (pow((abs(center.at<uchar>(ii, jj) - dm1.at<uchar>(0, jj))), 2)));
        }*/
    }
}

void Fuzzy_C_Means_S1::ud_FCM_S1(Mat &origFloatImg, int cluster_n, float a, Mat &center, Mat &U, vector<float>& obj_fcn) {
    ...
    udInit_FCM_S1(cluster_n, data_n, U);     // Initial fuzzy partition
    int ii = 0;
    ...
    for (ii = 0; ii < max_iter; ii++) {
        ...
        udStepFCM_S1(data, dataMean, U, cluster_n, a, expo, center, tempObjectiveFuncVal);
...
void Fuzzy_C_Means_S1::udStepFCM_S1(Mat &data, Mat &dataMean, Mat &U, int &clusterN, double a, int expo, Mat &center,
                                    float& obj_func) {
...
/*    float sum_1 = 0.0;
    ...
    Mat dist;
    udDist_FCM_S1(center, data, dm1, a, dist);
...
void Fuzzy_C_Means_S1::udInit_FCM_S1(int cluster_n, int data_n, Mat &U) {
...
void Fuzzy_C_Means_S1::obj_mat(Mat &center, Mat &data, Mat &dm1, Mat U, int expo, Mat &out1, Mat &out2) {
    Mat dm1Trans = dm1.t();
    Mat dm2 = dm1Trans.reshape(1, (dm1Trans.rows * dm1Trans.cols));
    dm1Trans.release();
...
void Fuzzy_C_Means_S1::neighborCalculation(Mat &dm1, std::vector<std::vector<int>> &result, int ii1) {
    int r = dm1.rows;
    int c = dm1.cols;
...
void Fuzzy_C_Means_S1::multiplyMat(std::vector<std::vector<int> > &Mat1, std::vector<std::vector<int> > &Mat2,
...
void Fuzzy_C_Means_S1::multiplyTwoFloatMat(Mat &Mat1, Mat &Mat2, Mat &result) {
    int mm = Mat1.rows;
    int pp = Mat2.cols;
    int nn = Mat2.rows;
...
void Fuzzy_C_Means_S1::multiplyOpenCVMat2(Mat &Mat1, Mat &Mat2, Mat &result) {
    int mm = Mat1.rows;
    int pp = Mat2.cols;
    int nn = Mat2.rows;
    result = Mat::zeros(mm, pp, CV_32FC1);
...
void Fuzzy_C_Means_S1::multiplyOpenCVMat(Mat &Mat1, Mat &Mat2, Mat &result) {
    int mm = Mat1.rows;
    int pp = Mat2.cols;
    int nn = Mat2.rows;
    result = Mat::zeros(mm, pp, CV_32FC1);
...
void Fuzzy_C_Means_S1::removeOneRow(std::vector<std::vector<int> > &matIn, int rowToRemove) {
    matIn.erase(matIn.begin() + rowToRemove); // erase the 10th row
}

void Fuzzy_C_Means_S1::removeManyRow(Mat &matIn, Mat &matOut, int rowToRemoveStart, int rowRemoveEnd) {

    if (rowToRemoveStart >= 0) // Copy everything above that one row.
    {
        cv::Rect rect(0, 0, matIn.cols, rowToRemoveStart);
        matIn(rect).copyTo(matOut(rect));
    }

    if (rowRemoveEnd < matIn.rows - 1) // Copy everything below that one row.
    {
        cv::Rect rect1(0, rowRemoveEnd + 1, matIn.cols, matIn.rows - rowRemoveEnd - 1);
        cv::Rect rect2(0, rowRemoveEnd, matIn.cols, matIn.rows - rowRemoveEnd - 1);
        matIn(rect1).copyTo(matOut(rect2));
    }
}

void Fuzzy_C_Means_S1::applyAlgoOnImage(Mat &imgOrig, Mat &matOut) {
    ...
    ud_FCM_S1(imgOrigFloat, 4, 4.2, center, U, obj_fcn);
 ...
 //   BasicAlgo::getInstance()->showImage(matOut);
    normImag.release();
}

void Fuzzy_C_Means_S1::compareTwoMatrixSimilarity(Mat& src1, Mat& src2, vector<int>& keepIndexes){
...
void Fuzzy_C_Means_S1:: matlab_reshape(const Mat &m, Mat& result, int new_row, int new_col, int new_ch)
{
