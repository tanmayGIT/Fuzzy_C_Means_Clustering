...
Fuzzy_C_Means_FGfsm_S1 *Fuzzy_C_Means_FGfsm_S1::instance = 0;

Fuzzy_C_Means_FGfsm_S1 *Fuzzy_C_Means_FGfsm_S1::getInstance() {
    if (!instance)
        instance = new Fuzzy_C_Means_FGfsm_S1();

    return instance;
}

Fuzzy_C_Means_FGfsm_S1::Fuzzy_C_Means_FGfsm_S1() {
}

Fuzzy_C_Means_FGfsm_S1::~Fuzzy_C_Means_FGfsm_S1() {
}

void Fuzzy_C_Means_FGfsm_S1::padMatrix(Mat &m, int n, Mat &out) {
...
void Fuzzy_C_Means_FGfsm_S1::padMatrix_1(Mat &m, Mat &outModified) {
...
void Fuzzy_C_Means_FGfsm_S1::udDist_FCM_M1(Mat &center, Mat &data, Mat &out) {
...
void Fuzzy_C_Means_FGfsm_S1::udInit_FCM_M1(int cluster_n, int data_n, Mat &U) {
...
void Fuzzy_C_Means_FGfsm_S1::step_FG_FCM (Mat &data, Mat& N, Mat &U, int clusterN, int expo, Mat &center, float& obj_func) {
    Mat mf;
    cv::pow(U, expo, mf);
...
void Fuzzy_C_Means_FGfsm_S1::lnwiFun(int ii, Mat& data2, std::vector<std::vector<int> >& rc, Mat& out){
    ...
    for (int iiWin = 0; iiWin < window.rows; iiWin++){
       int getIndex = (int) window.at<float>(iiWin,0);
        meanMe = meanMe + data.at<float>(getIndex,0);
    }
    meanMe = meanMe / window.rows;
    out = Mat::zeros(1,1,CV_32F);
    out.at<float>(0,0) = meanMe;
}


Mat Fuzzy_C_Means_FGfsm_S1::neighborCalculation(std::vector<std::vector<int> >&rc, int ii1) {
...
void Fuzzy_C_Means_FGfsm_S1::fg_FSM(Mat &origFloatImg, int cluster_n, Mat &center, Mat &U, vector<float>& obj_fcn) {
...
void Fuzzy_C_Means_FGfsm_S1::applyAlgoOnImage(Mat &imgOrig, Mat &matOut) {
...
void Fuzzy_C_Means_FGfsm_S1::calculateHistogram_2(const Mat& yuv420sp, const vector<float>& edgesBin, Mat& histogramBins,
...
void Fuzzy_C_Means_FGfsm_S1::compareTwoMatrixSimilarity(Mat& src1, Mat& src2, vector<int>& keepIndexes){
...
void Fuzzy_C_Means_FGfsm_S1:: matlab_reshape(const Mat &m, Mat& result, int new_row, int new_col, int new_ch)
{
...
void Fuzzy_C_Means_FGfsm_S1::multiplyTwo2DVector(std::vector<std::vector<int> > &Mat1, std::vector<std::vector<int> > &Mat2,
...
void Fuzzy_C_Means_FGfsm_S1::multiplyTwoFloatMat(Mat &Mat1, Mat &Mat2, Mat &result) {
    int mm = Mat1.rows;
    int pp = Mat2.cols;
    int nn = Mat2.rows;
...
void Fuzzy_C_Means_FGfsm_S1::multiplyOpenCVMat2(Mat &Mat1, Mat &Mat2, Mat &result) {
    int mm = Mat1.rows;
    int pp = Mat2.cols;
    int nn = Mat2.rows;
    result = Mat::zeros(mm, pp, CV_32FC1);
...
void Fuzzy_C_Means_FGfsm_S1::multiply_Float_Uchar_Mat(Mat &Mat1, Mat &Mat2, Mat &result) {
    int mm = Mat1.rows;
    int pp = Mat2.cols;
    int nn = Mat2.rows;
    result = Mat::zeros(mm, pp, CV_32FC1);
...
void Fuzzy_C_Means_FGfsm_S1::multiply_2_UChar_Mat(Mat &Mat1, Mat &Mat2, Mat &result) {
    int mm = Mat1.rows;
    int pp = Mat2.cols;
    int nn = Mat2.rows;
