...
Fuzzy_C_Means_FGfsm *Fuzzy_C_Means_FGfsm::instance = 0;

Fuzzy_C_Means_FGfsm *Fuzzy_C_Means_FGfsm::getInstance() {
    if (!instance)
        instance = new Fuzzy_C_Means_FGfsm();

    return instance;
}

Fuzzy_C_Means_FGfsm::Fuzzy_C_Means_FGfsm() {
}

Fuzzy_C_Means_FGfsm::~Fuzzy_C_Means_FGfsm() {
}

void Fuzzy_C_Means_FGfsm::padMatrix(Mat &m, int n, Mat &out) {
...
void Fuzzy_C_Means_FGfsm::padMatrix_1(Mat &m, Mat &outModified) {
...
void Fuzzy_C_Means_FGfsm::udDist_FCM_M1(Mat &center, Mat &data, Mat &out) {
...
void Fuzzy_C_Means_FGfsm::udInit_FCM_M1(int cluster_n, int data_n, Mat &U) {
...
void Fuzzy_C_Means_FGfsm::step_FG_FCM (Mat &data, Mat& N, Mat &U, int clusterN, int expo, Mat &center, float& obj_func) {
    Mat mf;
    cv::pow(U, expo, mf);
...
void Fuzzy_C_Means_FGfsm::lnwiFun(int ii, Mat& data2, std::vector<std::vector<int> >& rc, int rs, float rg, Mat& out){
    ...
    neighborCalculation(rc, ii);
    Mat neigh1 = createStructVar.neighbour1;
    Mat neigh2 = createStructVar.neighbour2;
    Mat lc = createStructVar.neighbour3;

    double sumMe = 0.0;
    for (int ik = 0; ik < neigh1.rows; ik++){
        int val1 = (int) neigh1.at<float>(ik,0);
        float val2 = data.at<float>(val1, 0);
        float val3 = data.at<float>(ii,0);
        sumMe = sumMe + pow(  (val2 - val3), 2);
    }
    sumMe = sumMe /8;
    double sigma = sqrt(sumMe);
    Mat S;
    lsm(data.at<float>(ii,0), lc, neigh1, neigh2, rs, rg, data, sigma, S);
    Mat dataRefined = Mat::zeros(neigh1.rows, data.cols, data.type());

    for (int iNeigh = 0; iNeigh < neigh1.rows; iNeigh++) {
        int tempIndex = (int)neigh1.at<float>(iNeigh, 0);
        dataRefined.at<float>(iNeigh,0) = data.at<float>(tempIndex,0);
    }
    Mat mulVect = (S.t() * dataRefined);

    double sumMeShow = (cv::sum(S)[0]);
    cv::divide(mulVect , (sumMeShow), out) ;
}

void Fuzzy_C_Means_FGfsm::lsm(float lcv, Mat lc, Mat neigh1,
                              Mat neigh2, int rs, float rg, Mat& data, float sigma, Mat& out){

    neigh2.col(0) = ((neigh2.col(0) - lc.at<float>(0,0)));
    neigh2.col(1) = ((neigh2.col(0) - lc.at<float>(0,1)));

    Mat p2 = Mat::zeros(neigh1.rows, 1, CV_32F);

    if (sigma == 0){
        Mat oneMatrix = Mat::ones(8, 1, CV_32F);
        Mat denomin = oneMatrix * rg;
        divide(1, denomin, p2);
    } else{
          for (int nn = 0; nn < neigh1.rows; nn++){
              auto tempVal = (int) neigh1.at<float>(nn,0);
              p2.at<float>(nn,0) = (float)(pow((data.at<float>(tempVal, 0) - lcv),2) / (rg * (pow(sigma,2))));
          }
    }
    Mat tempMat = abs(neigh2.t());

    double minVal = 0.0;
    double maxVal = 0.0;
    Point minLoc;
    Point maxLoc;
    Mat eachColMax = Mat::zeros(1,tempMat.cols, CV_32F );
    for (int jCol = 0; jCol < tempMat.cols; jCol++)
    {
        Mat eachCol = tempMat.col(jCol);
        minMaxLoc(eachCol, &minVal, &maxVal, &minLoc, &maxLoc);
        eachColMax.at<float>(0, jCol) = maxVal;
    }
    eachColMax = eachColMax / rs;
    p2.convertTo(p2,eachColMax.type());
    cv::exp((-(eachColMax.t() + p2)), out);
}


void Fuzzy_C_Means_FGfsm::neighborCalculation(std::vector<std::vector<int> >&rc, int ii1) {
    Mat out1;
    Mat out2;
    Mat out3;
    int r = rc[0][0];
   // int c = rc[1][0];
    ...
    float tempData[2] = {(float)(r1+1), (float)(c1+1)};
    out3 = cv::Mat(2, 1, CV_32F, tempData);

    out2 = temp;
    float refinedTemp[2] = {(float)(1), (float)(r+2)};
    Mat refinedMat = cv::Mat(2, 1, CV_32F, refinedTemp);

    Mat resultMul = temp * refinedMat;
    out1 = resultMul;

    //createStructVar = (NeighbourStruct*)malloc( sizeof( NeighbourStruct ) );
    createStructVar.neighbour1 = out1.clone();
    createStructVar.neighbour2 = out2.clone();
    createStructVar.neighbour3 = out3.clone();
}


void Fuzzy_C_Means_FGfsm::fg_FSM(Mat &origFloatImg, float rg, int cluster_n, Mat &center, Mat &U, vector<float>& obj_fcn) {
    ...
    for (int ii = 0; ii < data_n; ii++){
        Mat tempCalc;
        lnwiFun(ii, data2, rc, rs, rg, tempCalc);
...
void Fuzzy_C_Means_FGfsm::applyAlgoOnImage(Mat &imgOrig, Mat &matOut) {
    ...
    fg_FSM(imgOrigFloat, rg, cluster_n, center, U, obj_fcn);
...
void Fuzzy_C_Means_FGfsm::calculateHistogram_2(const Mat& yuv420sp, const vector<float>& edgesBin, Mat& histogramBins,
...
void Fuzzy_C_Means_FGfsm::compareTwoMatrixSimilarity(Mat& src1, Mat& src2, vector<int>& keepIndexes){
...
void Fuzzy_C_Means_FGfsm:: matlab_reshape(const Mat &m, Mat& result, int new_row, int new_col, int new_ch)
{
...
void Fuzzy_C_Means_FGfsm::multiplyTwo2DVector(std::vector<std::vector<int> > &Mat1, std::vector<std::vector<int> > &Mat2,
...
void Fuzzy_C_Means_FGfsm::multiplyTwoFloatMat(Mat &Mat1, Mat &Mat2, Mat &result) {
    int mm = Mat1.rows;
    int pp = Mat2.cols;
    int nn = Mat2.rows;
...
void Fuzzy_C_Means_FGfsm::multiplyOpenCVMat2(Mat &Mat1, Mat &Mat2, Mat &result) {
    int mm = Mat1.rows;
    int pp = Mat2.cols;
    int nn = Mat2.rows;
    result = Mat::zeros(mm, pp, CV_32FC1);
...
void Fuzzy_C_Means_FGfsm::multiply_Float_Uchar_Mat(Mat &Mat1, Mat &Mat2, Mat &result) {
    int mm = Mat1.rows;
    int pp = Mat2.cols;
    int nn = Mat2.rows;
    result = Mat::zeros(mm, pp, CV_32FC1);
...
void Fuzzy_C_Means_FGfsm::multiply_2_UChar_Mat(Mat &Mat1, Mat &Mat2, Mat &result) {
    int mm = Mat1.rows;
    int pp = Mat2.cols;
    int nn = Mat2.rows;
